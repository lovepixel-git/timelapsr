...
struct OnboardingView: View {
  ...
  var body: some View {
    VStack {
      Group {
        ...
        case .timeMultiplePage:
          card(
            title: "Define Your Time Multiple",
            subtitle: "Your time multiple is how much faster your output video is than real life",
            image: "OnboardingTimeMultiple", index: 2)
        case .filteringPage:
          card(
            title: "Filter out unwanted apps",
            subtitle: "Edit the enabled and disabled list to avoid recording unwanted apps",
            image: "OnboardingFilterApps", index: 3)
        case .cameraPage:
          card(
            title: "Record many screens and cameras",
            subtitle: "Click any camera or screen to record it", image: "OnboardingMultipleDevices",
            index: 4)
        case .getStarted:
          card(
  ...
  func card(title: LocalizedStringKey, subtitle: LocalizedStringKey, image: String, index: Int)
    -> some View
  {
    ZStack {
      Image(image)
